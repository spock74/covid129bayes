...
```{r}
bn.hc.wl.bl <- hc(s1, whitelist = wl, blacklist = bl)
training  <- bn.fit(bn.hc.wl.bl, s1)
training2 <- bn.fit(bn.hc.wl.bl, s1, method = "bayes", iss = 10)
predicted  <- predict(training,  node = "EVOLUCAO", data = s2, method = "bayes-lw")
predicted2 <- predict(training2, node = "EVOLUCAO", data = s2, method = "bayes-lw")
s2$PREDICAO  <- predicted
s2$PREDICAO2 <- predicted2

s2 <- na.omit(s2)

conf_mat1  <- confusion_matrix(targets = s2$EVOLUCAO,
                              predictions = s2$PREDICAO)
conf_mat2 <- confusion_matrix(targets = s2$EVOLUCAO,
                              predictions = s2$PREDICAO2)
par(mfrow = c(1, 2))
